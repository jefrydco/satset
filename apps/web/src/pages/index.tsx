...
const api = ky.create({ prefixUrl: `${API_HOST}/api`, headers: { [HEADER_APP_VERSION_KEY]: '1' } });
...
export default function Measure() {
  ...
  useEffect(() => {
    runStatus({ measureMongoId: router.query.measureId as string });
  }, [router.query])
  useEffect(() => {
    if (runResponseData) {
  ...
  useEffect(() => {
    if (
      ...
      clearInterval(intervalId.current);
      if (!router.query.measureId) {
        router.push(`${router.asPath}?measureId=${runStatusResponseData.id}`)
      }
    }
